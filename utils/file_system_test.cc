...
TEST(FileSystem, create_file) {
    ...
    if (utils::FileSystem::CreateDirectory(test_dir)) {
    ...
    if (utils::FileSystem::CheckPathExist(test_dir)) {
        LOG(INFO) << "[Detection] Directory exists: " << test_dir;
    }

    // 3. Recursively delete directories
    if (utils::FileSystem::RemoveDirectory("./test")) {  // delete parent folder
        LOG(INFO) << "[CLEANUP] Directory deleted: ./test";
    }
    const char* test_path = "/dev/shm/demo/0/1";
    LOG(INFO) << "Parent folder name is: " << utils::FileSystem::GetDirName(test_path);
}
